...
# One-command deployment for nplus_project STAGING environment.
# Steps:
# 1) git pull --ff-only
# 2) python manage.py migrate
# 3) restart gunicorn-staging and nginx
# 4) health checks (local socket + external URL)

APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BRANCH="${DEPLOY_BRANCH:-main}"
GUNICORN_SERVICE="${GUNICORN_SERVICE:-gunicorn-staging}"
NGINX_SERVICE="${NGINX_SERVICE:-nginx}"
SOCKET_PATH="${SOCKET_PATH:-/run/nplus_project_staging/nplus_project_staging.sock}"
SOCKET_WAIT_SECONDS="${SOCKET_WAIT_SECONDS:-20}"
HEALTH_URL="${HEALTH_URL:-https://staging.npbpm.cn/}"
...
log "Staging deploy started in $APP_DIR"
...
if wait_for_socket "$SOCKET_PATH" "$SOCKET_WAIT_SECONDS"; then
  ...
  log "Recent gunicorn-staging logs"
...
log "Staging deploy finished successfully"
